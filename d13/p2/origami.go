...
func get_size(positions map[Point]bool) (int, int) {
    xs, ys := 0, 0
...
func print_map(positions map[Point]bool) {
    xs, ys := get_size(positions)
    for y := 0; y <= ys; y++ {
        for x := 0; x <= xs; x++ {
            if _, marked := positions[Point{x, y}]; !marked {
                fmt.Print(".")
            } else {
                fmt.Print("#")
            }
