...
func take_steps(e_levels [][]int, steps int) ([][]int, int) {
    ...
    for x := 0; x < steps; x++ {
        ...
        count_flashed += len(xth_flashes)
    }
    return e_levels, count_flashed
}

func main() {
    init_levels := parse_input("../inputs/input.in")
    c_flashed := 0
    _, c_flashed = take_steps(init_levels, 100)
    fmt.Println(c_flashed)
}
