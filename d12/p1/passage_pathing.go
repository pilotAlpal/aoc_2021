...
func find_paths(graph map[string][]string, start, end string, current_path []string, smalls_visited map[string]bool, paths int) int {
    ...
    for _, p := range graph[start] {
        if _, visited := smalls_visited[p]; !visited {
            ...
            paths = find_paths(graph, p, end, current_path, smalls_visited, paths)
...
func get_paths(graph map[string][]string, start, end string) int {
    ...
    scv[start] = true
    return find_paths(graph, start, end, []string{}, scv, 0)
}
